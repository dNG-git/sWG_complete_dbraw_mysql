...
if (defined ("CLASS_directPdoMysql")) { $g_continue_check = false; }
...
*             Mozilla Public License, v. 2.0
*/
class directPdoMysql extends directVirtualClass
{
/**
...
/**
	* Constructor (PHP5) __construct (directPdoMysql)
	*
	* @since v0.1.00
*/
	public function __construct ()
	{
		...
		if (USE_debug_reporting) { direct_debug (5,"sWG/#echo(__FILEPATH__)# -db->__construct (directPdoMysql)- (#echo(__LINE__)#)"); }
		...
		if (isset ($direct_settings['db_synchronisation']))
		{
			...
			{
				if (!$direct_globals['basic_functions']->includeFile ($direct_settings['path_system']."/functions/swg_dbsync.php",1)) { trigger_error ("sWG/#echo(__FILEPATH__)# -db->__construct (directPdoMysql)- (#echo(__LINE__)#) reporting: Manual synchronisation trigger unavailable",E_USER_WARNING); }
			}
		}
...
/**
	* Destructor (PHP5) __destruct (directPdoMysql)
	*
	* @since v0.1.00
*/
...
*/
	public function connect ()
	{
		...
		elseif (strlen ($direct_settings['db_dsa']))
		{
			...
			try
			{
				if ($direct_settings['db_peristent']) { $this->resource = new /*#ifdef(PHP5n) */\PDO/* #*//*#ifndef(PHP5n):PDO:#*/ ($direct_settings['db_dsa'],$direct_settings['db_username'],$direct_settings['db_password'],(array ($this->pdo['persistent'] => true,$this->pdo['errormode'] => $this->pdo['errormode_silent'],$this->pdo['timeout'] => $direct_settings['timeout_core']))); }
				else { $this->resource = new /*#ifdef(PHP5n) */\PDO/* #*//*#ifndef(PHP5n):PDO:#*/ ($direct_settings['db_dsa'],$direct_settings['db_username'],$direct_settings['db_password'],(array ($this->pdo['persistent'] => false,$this->pdo['errormode'] => $this->pdo['errormode_silent'],$this->pdo['timeout'] => $direct_settings['timeout_core']))); }
			}
		...
		if (is_object ($this->resource))
		{
			...
			if ($f_continue_check)
			{
				...
				if ((($f_data['type'] == "insert")||($f_data['type'] == "replace")||($f_data['type'] == "update"))&&($f_data['set_attributes']))
				{
					...
					if (isset ($f_xml_node_array['sqlvalues'])) { $this->query_cache .= " SET ".($this->queryBuildSetAttributes ($f_xml_node_array['sqlvalues'])); }
				}
			...
			foreach ($f_requirements_array as $f_requirement_array)
			{
				if (is_array ($f_requirement_array))
				{
					...
					elseif ($f_requirement_array['value'] != "*")
					{
						...
						if (isset ($f_requirement_array['attributes']['null'])) { $f_return .= (($f_requirement_array['attributes']['operator'] == "!=") ? " IS NOT NULL" : " IS NULL"); }
						...
						elseif ($f_requirement_array['attributes']['type'] == "string") { $f_return .= " {$f_requirement_array['attributes']['operator']} ''"; }
						else { $f_return .= (($f_requirement_array['attributes']['operator'] == "!=") ? " IS NOT NULL" : " IS NULL"); }
					}
				}
			}
		}
		...
		if (is_object ($this->resource))
		{
			...
			else
			{
				...
				if (($f_answer == "co")||(!$f_result_object)||(!is_object ($f_result_object)))
				{
					...
					else { trigger_error ("sWG/#echo(__FILEPATH__)# -db->queryExec ()- (#echo(__LINE__)#) reporting: ANSI ".($this->resource->errorCode ()),E_USER_ERROR); }
				}
...
*/
	public function optimize ($f_table)
	{
		...
		if (is_object ($this->resource)) { $f_return = $this->queryExec ("co","OPTIMIZE TABLE ".$f_table); }
...
*/
	public function transactionBegin ()
	{
		...
		if ($direct_settings['db_transaction_supported'])
		{
			...
			if (($direct_settings['db_transaction_supported'])&&(is_object ($this->resource)))
			{
				if ($this->transactions > 1) { $f_return = ($direct_settings['db_nested_transactions_supported'] ? $this->queryExec ("co","SAVEPOINT ".(1 + $this->transactions)) : true); }
				else { $f_return = $this->queryExec ("co","BEGIN WORK"); }
...
*/
	public function transactionCommit ()
	{
		...
		if ($direct_settings['db_transaction_supported'])
		{
			...
			{
				if ($this->transactions > 1) { $f_return = ($direct_settings['db_nested_transactions_supported'] ? $this->queryExec ("co","RELEASE SAVEPOINT ".$this->transactions) : true); }
				else { $f_return = $this->queryExec ("co","COMMIT"); }
...
*/
	public function transactionRollback ()
	{
		...
		if ($direct_settings['db_transaction_supported'])
		{
			...
			if (($this->transactions > 0)&&(is_object ($this->resource)))
			{
				if ($this->transactions > 1) { $f_return = ($direct_settings['db_nested_transactions_supported'] ? $this->queryExec ("co","ROLLBACK TO SAVEPOINT ".$this->transactions) : true); }
				else { $f_return = $this->queryExec ("co","ROLLBACK"); }
...
define ("CLASS_directPdoMysql",true);
