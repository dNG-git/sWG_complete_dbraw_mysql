...
*             Mozilla Public License, v. 2.0
*/
class directMysql extends directVirtualClass
{
/**
...
/**
	* Constructor (PHP5) __construct (directMysql)
	*
	* @since v0.1.00
*/
	/*#ifndef(PHP4) */public /* #*/function __construct ()
	{
		...
		if (USE_debug_reporting) { direct_debug (5,"sWG/#echo(__FILEPATH__)# -db->__construct (directMysql)- (#echo(__LINE__)#)"); }
		...
		if (isset ($direct_settings['db_synchronisation']))
		{
			...
			{
				if (!$direct_globals['basic_functions']->includeFile ($direct_settings['path_system']."/functions/swg_dbsync.php",1)) { trigger_error ("sWG/#echo(__FILEPATH__)# -db->__construct (directMysql)- (#echo(__LINE__)#) reporting: Manual synchronisation trigger unavailable",E_USER_WARNING); }
			}
		}
...
/*#ifdef(PHP4):
/**
	* Constructor (PHP4) directMysql (directMysql)
	*
	* @since v0.1.00
*\/
	function directMysql () { $this->__construct (); }
:#\n*/
/**
	* Destructor (PHP5) __destruct (directMysql)
	*
	* @since v0.1.00
*/
	...
	/*#ifndef(PHP4) */public /* #*/function connect ()
	{
		...
		elseif ((function_exists ("mysql_pconnect"))&&(strlen ($direct_settings['db_dbname'])))
		{
			...
			if (is_resource (($this->resource)))
			{
				...
				else { trigger_error ("sWG/#echo(__FILEPATH__)# -db->connect ()- (#echo(__LINE__)#) reporting: ".(mysql_error ($this->resource)),E_USER_ERROR); }
			}
			else { trigger_error ("sWG/#echo(__FILEPATH__)# -db->connect ()- (#echo(__LINE__)#) reporting: Wrong password for the specified username",E_USER_ERROR); }
		}
	...
	/*#ifndef(PHP4) */public /* #*/function queryExec ($f_answer,$f_query)
	{
		...
		if (is_resource ($this->resource))
		{
			...
			elseif ((!$f_result_pointer)||(!is_resource ($f_result_pointer)))
			{
				...
				else { trigger_error ("sWG/#echo(__FILEPATH__)# -db->queryExec ()- (#echo(__LINE__)#) reporting: ".(mysql_error ($this->resource)),E_USER_ERROR); }
			}
...
*/
	/*#ifndef(PHP4) */public /* #*/function optimize ($f_table)
	{
		...
		if (is_resource ($this->resource)) { $f_return = $this->queryExec ("co","OPTIMIZE TABLE ".$f_table); }
